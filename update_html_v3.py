...
def update_html_with_images_and_pros_cons():
    ...
    for row in rows:
        ...
        # Create pros/cons cell with simple string construction
        proscons_cell = soup.new_tag('td')
        proscons_html = '<div style="font-size:0.8rem;">'
        
        if pros:
            pros_text = '<br>'.join([f"✓ {pro}" for pro in pros])
            proscons_html += f'<div style="color:#16a34a;">{pros_text}</div>'
        
        if cons:
            cons_text = '<br>'.join([f"✗ {con}" for con in cons])
            proscons_html += f'<div style="color:#ef4444;margin-top:4px;">{cons_text}</div>'
        
        proscons_html += '</div>'
        proscons_cell.append(BeautifulSoup(proscons_html, 'html.parser'))
